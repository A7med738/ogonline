-- Add shop view counts system (Safe Version for Production)
-- This migration adds view count functionality for mall shops
-- This version is safe for production and handles existing objects gracefully

-- Add view count column to mall_shops table
ALTER TABLE mall_shops 
ADD COLUMN IF NOT EXISTS view_count INTEGER DEFAULT 0;

-- Create shop_views table for detailed view tracking (only if it doesn't exist)
...
-- Create policy to allow reading shop views (for analytics) - only if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'shop_views' 
    AND policyname = 'Allow reading shop views'
  ) THEN
...
-- Create policy to allow inserting shop views (for tracking) - only if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'shop_views' 
    AND policyname = 'Allow inserting shop views'
  ) THEN
