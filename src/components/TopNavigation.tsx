...
export const TopNavigation: React.FC<TopNavigationProps> = ({ isAdmin }) => {
  ...
  const menuItems = [
    ...
    { icon: Newspaper, label: 'أخبار المدينة', path: '/news' },
    { icon: Shield, label: 'أرقام الشرطة', path: '/police' },
    { icon: Building, label: 'جهاز المدينة', path: '/city' },
    ...(user ? [
      { icon: User, label: 'الملف الشخصي', path: '/profile' },
      ...(isAdmin ? [{ icon: Settings, label: 'الإدارة', path: '/admin' }] : [])
    ] : [{ icon: User, label: 'تسجيل الدخول', path: '/auth' }])
  ];
                ...
                {user && (
                  <>
                    ...
                    <button
                      ...
                      className="w-full flex items-center px-4 py-3 text-right hover:bg-gray-50 transition-colors text-red-600"
                    >
