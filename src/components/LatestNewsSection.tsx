  ...
  return (
    <div className="mt-12 animate-fade-in">
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-muted-foreground text-sm">ابق على اطلاع بآخر المستجدات والأحداث</p>
      </div>
      ...
      <div className="max-w-sm mx-auto relative">
        {latestNews.length > 0 && (
          <div className="relative overflow-hidden">
            ...
            >
              <div
                className="glass-card rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-glass cursor-pointer"
                onClick={() => navigate('/news')}
              >
                {/* News Image */}
                {latestNews[currentIndex]?.image_url && (
                  <div className="h-32 overflow-hidden">
                    <img
                      src={latestNews[currentIndex].image_url}
            ...
            {latestNews.length > 1 && (
              <>
                <Button
                  ...
                  size="sm"
                  onClick={handlePrev}
                  className="absolute right-2 top-1/2 -translate-y-1/2 h-8 w-8 p-0 bg-black/20 hover:bg-black/40 text-white border-0"
                ...
                <Button
                  ...
                  size="sm"
                  onClick={handleNext}
                  className="absolute left-2 top-1/2 -translate-y-1/2 h-8 w-8 p-0 bg-black/20 hover:bg-black/40 text-white border-0"
                  disabled={isAnimating}
                >
      ...
      <div className="text-center mt-6">
        <Button
          onClick={() => navigate('/news')}
          className="bg-gradient-primary hover:opacity-90 transition-opacity text-sm px-6 py-2"
        >
          عرض جميع الأخبار
        </Button>
      </div>
