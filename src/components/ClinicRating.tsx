...
import { getClinicRatings, addClinicRating, getClinicRatingStats } from '@/utils/clinicRatingsStorage';
...
interface ClinicRatingProps {
  ...
  totalRatings?: number;
  onRatingUpdate?: (clinicId: string, averageRating: number, totalRatings: number) => void;
}
...
const ClinicRating: React.FC<ClinicRatingProps> = ({ 
  ...
  const [ratings, setRatings] = useState<ClinicRating[]>([]);
  ...
  const loadRatings = async () => {
    try {
      ...
      if (error) {
        console.log('Clinic ratings table not found, using local storage');
        // استخدام التخزين المحلي
        const localRatings = getClinicRatings(clinicId);
  ...
  const handleSubmitRating = async () => {
    ...
    try {
      ...
      if (error) {
        console.log('Clinic ratings table not found, storing locally');
        // تخزين محلي مؤقت
        const newRatingData = addClinicRating({
          clinic_id: clinicId,
          user_name: userName.trim(),
          rating: newRating,
          comment: newComment.trim()
        ...
        if (onRatingUpdate) {
          const stats = getClinicRatingStats(clinicId);
          onRatingUpdate(clinicId, stats.averageRating, stats.totalRatings);
        }
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  <DialogTitle>تقييم {clinicName}</DialogTitle>
                </DialogHeader>
                <div className="space-y-4">
                  <div>
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      placeholder="شاركنا رأيك في هذه العيادة..."
                      rows={3}
                    />
                  </div>
                  <Button 
