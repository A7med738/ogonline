...
import { Trash2, Edit, Plus, MapPin, Clock, Phone } from 'lucide-react';
...
const ElectricityCompanyManagement = () => {
  const [branches, setBranches] = useState<ElectricityCompany[]>([]);
  const [loading, setLoading] = useState(true);
  const [editingBranch, setEditingBranch] = useState<ElectricityCompany | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingBranch) {
        ...
        if (error) throw error;
        toast.success('تم تحديث فرع شركة الكهرباء بنجاح');
      } else {
        const { error } = await supabase
          .from('electricity_company')
          .insert([branchData]);
        ...
        if (error) throw error;
        toast.success('تم إضافة فرع شركة الكهرباء بنجاح');
      }
      ...
      console.error('Error saving electricity company branch:', error);
      toast.error('حدث خطأ أثناء حفظ فرع شركة الكهرباء');
    }
  };

  const handleEdit = (branch: ElectricityCompany) => {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('هل أنت متأكد من حذف فرع شركة الكهرباء؟')) return;
    ...
    try {
      ...
      if (error) throw error;
      toast.success('تم حذف فرع شركة الكهرباء بنجاح');
      fetchBranches();
    } catch (error) {
      console.error('Error deleting electricity company branch:', error);
      toast.error('حدث خطأ أثناء حذف فرع شركة الكهرباء');
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold">إدارة فروع شركة الكهرباء</h2>
      ...
      {showForm && (
        <Card>
          <CardHeader>
            <CardTitle>{editingBranch ? 'تعديل فرع شركة الكهرباء' : 'إضافة فرع جديد لشركة الكهرباء'}</CardTitle>
          </CardHeader>
          <CardContent>
...
export default ElectricityCompanyManagement;
