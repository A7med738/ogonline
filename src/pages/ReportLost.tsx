...
const ReportLost: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title.trim()) return toast.error("يرجى إدخال عنوان للمفقود");
    if (!category) return toast.error("يرجى اختيار ما فقدت");
    ...
    try {
      ...
      if (media.length > 0 && media[0].file) {
        ...
        const fileName = `lost-items/${Date.now()}-${file.name}`;
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-6">
          <h3 className="text-xl md:text-2xl font-bold">إبلاغ عن شيء مفقود</h3>
        </div>

        <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-5">
          <GlassCard className="p-4 space-y-3">
            <div className="space-y-2">
              <Label>عنوان المفقود *</Label>
              <Input
                placeholder="مثال: قطة شيرازية بيضاء، محفظة جلدية سوداء"
                value={title}
            ...
            <div className="space-y-2">
              <Label>آخر مكان تتذكر وجوده فيه *</Label>
              <LocationPicker
            ...
            <div className="space-y-2">
              <Label>وصف تفصيلي وصورة (إن وجدت)</Label>
              <Textarea
                placeholder="مثال: قطة شيرازية بيضاء اسمها لوسي، عمرها 3 سنوات، لها علامة مميزة خلف الأذن اليسرى"
                value={description}
