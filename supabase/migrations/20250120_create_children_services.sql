...
COMMENT ON TABLE children_services IS 'جدول خدمات الأطفال والأنشطة الترفيهية';
COMMENT ON COLUMN children_services.name IS 'اسم الخدمة أو المكان';
COMMENT ON COLUMN children_services.type IS 'نوع الخدمة: playground (ملاهي), kids_area (كيدز ايريا), entertainment (أنشطة ترفيهية), games (ألعاب), art (فنون), music (موسيقى)';
COMMENT ON COLUMN children_services.address IS 'عنوان المكان';
COMMENT ON COLUMN children_services.phone IS 'رقم الهاتف الرئيسي';
COMMENT ON COLUMN children_services.phone2 IS 'رقم الهاتف الثانوي';
COMMENT ON COLUMN children_services.whatsapp IS 'رقم الواتساب';
COMMENT ON COLUMN children_services.facebook_url IS 'رابط صفحة الفيسبوك';
COMMENT ON COLUMN children_services.google_maps_url IS 'رابط خرائط جوجل';
COMMENT ON COLUMN children_services.website IS 'رابط الموقع الإلكتروني';
COMMENT ON COLUMN children_services.description IS 'وصف الخدمة';
COMMENT ON COLUMN children_services.image_url IS 'رابط صورة المكان';
COMMENT ON COLUMN children_services.logo_url IS 'رابط شعار المكان';
COMMENT ON COLUMN children_services.is_active IS 'حالة النشاط';
