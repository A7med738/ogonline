...
CREATE TABLE IF NOT EXISTS school_categories (
    ...
    description_ar TEXT,
    description_en TEXT,
    color VARCHAR(7) DEFAULT '#3B82F6', -- لون الفئة
...
CREATE INDEX IF NOT EXISTS idx_school_categories_name_en ON school_categories(name_en);
...
-- إضافة فئة category_id إلى جدول schools
ALTER TABLE schools 
...
-- إدراج الفئات الأساسية (مع التحقق من عدم وجودها)
INSERT INTO school_categories (name_ar, name_en, description_ar, description_en, color, icon, sort_order)
SELECT 'مدارس حكومية', 'Government Schools', 'المدارس الحكومية التابعة لوزارة التربية والتعليم', 'Government schools under the Ministry of Education', '#10B981', 'school', 1
WHERE NOT EXISTS (SELECT 1 FROM school_categories WHERE name_ar = 'مدارس حكومية');

INSERT INTO school_categories (name_ar, name_en, description_ar, description_en, color, icon, sort_order)
SELECT 'مدارس ناشونال', 'National Schools', 'المدارس الناشونال الخاصة', 'Private national schools', '#F59E0B', 'graduation-cap', 2
WHERE NOT EXISTS (SELECT 1 FROM school_categories WHERE name_ar = 'مدارس ناشونال');

INSERT INTO school_categories (name_ar, name_en, description_ar, description_en, color, icon, sort_order)
SELECT 'مدارس انترناشونال', 'International Schools', 'المدارس الدولية', 'International schools', '#8B5CF6', 'globe', 3
WHERE NOT EXISTS (SELECT 1 FROM school_categories WHERE name_ar = 'مدارس انترناشونال');

-- تحديث المدارس الموجودة لتكون حكومية افتراضياً
UPDATE schools 
SET category_id = (SELECT id FROM school_categories WHERE name_ar = 'مدارس حكومية' LIMIT 1)
WHERE category_id IS NULL;

-- إنشاء سياسة RLS للفئات
