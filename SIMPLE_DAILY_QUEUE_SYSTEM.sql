-- نظام طابور يومي بسيط لكل دكتور - نظام احجزلي
...
-- 4. إنشاء جدول لإدارة الطابور اليومي لكل عيادة
CREATE TABLE IF NOT EXISTS book_service_daily_queues (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  clinic_id UUID REFERENCES book_service_clinics(id) ON DELETE CASCADE,
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(clinic_id, queue_date) -- طابور واحد فقط لكل عيادة في اليوم
);

-- 5. إنشاء دالة لحساب رقم الطابور التالي للعيادة في اليوم
CREATE OR REPLACE FUNCTION get_next_daily_queue_number(clinic_uuid UUID, appointment_date DATE)
RETURNS INTEGER AS $$
DECLARE
  next_num INTEGER;
BEGIN
  -- الحصول على رقم الطابور التالي للعيادة في هذا اليوم
...
-- 6. إنشاء دالة لحساب موضع المريض في الطابور اليومي
CREATE OR REPLACE FUNCTION calculate_daily_queue_position(clinic_uuid UUID, appointment_uuid UUID, appointment_date DATE)
RETURNS INTEGER AS $$
DECLARE
  position INTEGER;
BEGIN
  -- حساب عدد المرضى المنتظرين قبل هذا المريض في نفس اليوم
  SELECT COUNT(*) + 1
  INTO position
  FROM book_service_appointments 
  WHERE clinic_id = clinic_uuid 
...
-- 7. إنشاء دالة لتحديث الطابور اليومي عند إضافة موعد جديد
CREATE OR REPLACE FUNCTION update_daily_queue_on_appointment()
RETURNS TRIGGER AS $$
BEGIN
  -- تحديث رقم الطابور للموعد الجديد
  NEW.queue_number := get_next_daily_queue_number(NEW.clinic_id, NEW.appointment_date);
  
  -- حساب موضع المريض في الطابور اليومي
  NEW.queue_position := calculate_daily_queue_position(NEW.clinic_id, NEW.id, NEW.appointment_date);
  
  -- تحديث أو إنشاء الطابور اليومي للعيادة
  INSERT INTO book_service_daily_queues (clinic_id, queue_date, current_queue_number, total_patients_today)
  VALUES (NEW.clinic_id, NEW.appointment_date, NEW.queue_number, 1)
...
-- 8. إنشاء trigger لتحديث الطابور اليومي تلقائياً
DROP TRIGGER IF EXISTS trigger_update_daily_queue ON book_service_appointments;
CREATE TRIGGER trigger_update_daily_queue
  BEFORE INSERT ON book_service_appointments
  FOR EACH ROW
  EXECUTE FUNCTION update_daily_queue_on_appointment();

-- 9. حذف البيانات الموجودة أولاً (إذا كانت موجودة)
DELETE FROM book_service_daily_queues WHERE clinic_id IN (SELECT id FROM book_service_clinics);
...
-- 10. إدراج بيانات تجريبية للمراكز الصحية
...
-- 11. إدراج بيانات تجريبية للعيادات
...
ALTER TABLE book_service_daily_queues ENABLE ROW LEVEL SECURITY;

-- 13. حذف السياسات الموجودة أولاً (إذا كانت موجودة)
...
DROP POLICY IF EXISTS "الطوابير اليومية مرئية للجميع" ON book_service_daily_queues;
DROP POLICY IF EXISTS "الطوابير اليومية قابلة للتحديث للمديرين" ON book_service_daily_queues;

-- 14. إنشاء سياسات الأمان للمراكز الصحية
...
-- 15. إنشاء سياسات الأمان للعيادات
...
-- 16. إنشاء سياسات الأمان للحجوزات
...
-- 17. إنشاء سياسات الأمان للطوابير اليومية
CREATE POLICY "الطوابير اليومية مرئية للجميع" ON book_service_daily_queues
  FOR SELECT USING (true);

CREATE POLICY "الطوابير اليومية قابلة للتحديث للمديرين" ON book_service_daily_queues
  FOR UPDATE USING (auth.role() = 'service_role');

-- 18. إنشاء فهارس لتحسين الأداء
...
CREATE INDEX IF NOT EXISTS idx_book_service_daily_queues_clinic_date ON book_service_daily_queues(clinic_id, queue_date);

-- تم إنشاء نظام الطابور اليومي البسيط بنجاح! ✅
